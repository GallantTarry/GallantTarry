...
到了这里几乎所有的配置已经完成了，但是到这里还是没有完全完成一个项目框架的基本建立，那么我还要推荐一个非常经典的**三层架构（Three-Tier Architecture）**，现在的很多架构其实都来自于三层架构随着互联网的发展，项目规模越来越庞大，我们后来又在三层架构的基础上演进出了DDD（领域驱动设计）、微服务架构（Microservices）甚至Serverless架构。但万变不离其宗，无论架构如何演进，其核心思想永远是“**关注点分离**”与“**职责单一**”。
